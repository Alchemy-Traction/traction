...
module.exports = class Habit {
  ...
  static async findHabitByReminderTime() {
    const { rows } = await pool.query(
      ...
      JOIN users
      ON users.id=habits.user_id 
      WHERE reminder_time > LOCALTIME -INTERVAL '8 HOURS 5 MINUTES'
      AND reminder_time < LOCALTIME -INTERVAL '7 HOURS 55 MINUTES';`,
    );
    return rows;
  }
  ...
  static async confirmLabelsbyUserId(userId, mungedLabels) {
    const { rows } = await pool.query(
      `SELECT * 
        FROM habits 
        WHERE user_id = $1
