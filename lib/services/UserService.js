...
module.exports = class UserService {
  ...
  static async create(phoneNumber) {
    ...
    const user = await User
      .insert(phoneNumber)
      .then((user) => sendSms(user.phoneNumber, 'Thanks for signing up to Tracktion! You are on your way to building healthy habits! Let\'s start with a picture, send me a selfie!'));
  ...
  static async addSelfie(phoneNumber, userPhotoUrl) {
    const user = await User
      ...
      .then((user) => User.updateSelfie(user.id, userPhotoUrl))
